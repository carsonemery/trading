...
# Analysis: Compare field_list with actual_fields
def analyze_field_differences():
    """Compare our field_list with actual fields in comp.fundq"""
    field_set = set(field_list)
    actual_set = set(actual_fields)
    
    # Fields in our list but NOT in actual database
    missing_from_db = field_set - actual_set
    
    # Fields in database but NOT in our list (we might want)
    not_in_our_list = actual_set - field_set
    
    print("="*80)
    print("FIELD COMPARISON ANALYSIS")
    print("="*80)
    print(f"\nOur field_list: {len(field_list)} fields")
    print(f"Actual comp.fundq: {len(actual_fields)} fields")
    print(f"\nFields in OUR list but NOT in database: {len(missing_from_db)}")
    print(f"Fields in database but NOT in our list: {len(not_in_our_list)}")
    
    if missing_from_db:
        print("\n" + "="*80)
        print("THESE FIELDS DON'T EXIST IN comp.fundq:")
        print("="*80)
        print("(These are causing the query to fail!)\n")
        for field in sorted(missing_from_db):
            print(f"  - {field}")
    
    if not_in_our_list:
        print("\n" + "="*80)
        print("FIELDS IN DATABASE WE'RE NOT PULLING:")
        print("="*80)
        print()
        for field in sorted(not_in_our_list)[:50]:  # Show first 50
            print(f"  - {field}")
        if len(not_in_our_list) > 50:
            print(f"\n  ... and {len(not_in_our_list) - 50} more")
...
def test_null_fields_across_sectors():
    ...
    # Original test stocks with sector info (commented out for reference)
    # test_stocks = {
    #     'AAPL': 'Technology (Large Cap)',
    #     'JPM': 'Financials (Large Cap)',
    #     'DUK': 'Utility (Large Cap)',
    #     'XOM': 'Energy (Large Cap)',
    #     'WMT': 'Consumer/Retail (Large Cap)',
    #     'JNJ': 'Healthcare (Large Cap)',
    #     'BA': 'Industrials (Large Cap)',
    #     'REXR': 'REIT (Mid Cap)',
    #     'AES': 'Utility/Power (Mid Cap)'
    # }
    
    # Edge cases: diverse sectors, sizes, and delisted companies
    edge_cases_list = [
        # Core diverse test set
        'AAPL', 'JPM', 'DUK', 'XOM', 'WMT', 'JNJ', 'BA', 'REXR', 'AES',
        # Delisted companies
        'TWTR', 'SHLDQ', 'NKLAQ', 'HMNY',
        # Additional edge cases
        'META', 'BLL', 'BALL', 'FB', 'AI', 'T', 'PTWO',
        'SBC', 'TWX', 'AOL', 'WBD', 'HWP', 'HPQ', 'NRXP',
